...
################################################################################
# READ DATA AND GET LAD AND LSOA LISTS

stage1list <- readRDS("./data/stage1list.RDS")
listlad <- names(stage1list)
listlsoa <- unlist(
  sapply(stage1list, function(x) x[["lsoatmeanper"]][["LSOA11CD"]]), 
  use.names = FALSE)
...
argvar <- list(
  fun = varfun,
  knots = c(16.40445, 19.92883), # From quantile(datatmean$tmean, varper/100, na.rm = T)
  Bound = c(3.09093, 28.89676) # From range(datatmean$tmean, na.rm = T)
)
tper <- unlist(lapply(seq(stage1list), function(i) stage1list[[i]][["ladtmeanper"]]), 
               use.names = FALSE)
bvar <- do.call(onebasis,c(list(x=tper), argvar))
...
for (c in seq(setcause)) {
  ...
  pooled_results_list[a] <- 
    ...
    by=0.1, cen= 16.40445 # From quantile(datatmean$tmean, 50/100, na.rm = T)
    ))
  }
